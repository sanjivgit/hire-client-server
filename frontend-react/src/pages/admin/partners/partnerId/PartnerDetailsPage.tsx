...
import IframeModal from "@/components/partners/IFrameModal"
...
export default function PartnerDetailPage() {
  ...
  const [blobUrl, setBlobUrl] = useState<{ url: string | null, fileName: string }>({
    url: null,
    fileName: "",
  });
  ...
  const fetchFile = async ({ fileId, fileName }: { fileId: string, fileName: string }) => {
    const token = localStorage.getItem('token');
    const response = await fetch(`${import.meta.env.VITE_PUBLIC_API_URL + FILES.FILE(fileId)}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
    const blob = await response.blob();
    setBlobUrl({ url: URL.createObjectURL(blob), fileName });
  };
  ...
  return (
    <>
                  ...
                  {partner.additionalDocument && partner.aadharImage ? (
                    <div className="space-y-3">
                      <div className="flex items-center justify-between rounded-md border p-3">
                        ...
                        <div className="flex items-center gap-2">
                          <Button variant="outline" size="sm" onClick={() => fetchFile({ fileId: partner.aadharImage.toString(), fileName: "Aadhar Card" })}>
                      ...
                      <div className="flex items-center justify-between rounded-md border p-3">
                        ...
                        <div className="flex items-center gap-2">
                          <Button variant="outline" size="sm" onClick={() => fetchFile({ fileId: partner.additionalDocument.toString(), fileName: "Additional Document" })}>
      ...
      <IframeModal
        ...
        isOpen={!!blobUrl?.url}
        setIsOpen={() => {
          setBlobUrl({ url: null, fileName: "" });
        }}
        title={`Preview: ${blobUrl?.fileName}`}
      />
    </>
  )
}